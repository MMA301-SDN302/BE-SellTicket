...
const getCarByRoute = async (startLocationName, endLocationName, timeStart) => {
  try {
    ...
    const userDirection = userStartStop.stop_id < userEndStop.stop_id ? "NORTHBOUND" : "SOUTHBOUND";
    ...
    if (!routes || routes.length === 0) {
      throw new Error("Không tìm thấy tuyến xe nào trong khoảng thời gian này.");
    }
    ...
    for (let route of routes) {
      ...
      const routeDirection = routeStartStop.stop_id < routeEndStop.stop_id ? "NORTHBOUND" : "SOUTHBOUND";
